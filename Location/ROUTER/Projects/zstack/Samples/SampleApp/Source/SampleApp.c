...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
 ...
 /* 
  if ( events & USER_EVT )//由于定义了p2p的目的节点为协调器，所以此处设置的是锚节点发送数据,使用自定义函数
  {
...
* @return  none
*/
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )//在本函数中应该对接收到的盲节点的RSSI值信息处理
{

  
  switch ( pkt->clusterId )
  {
    //本句确定只有在路由器中才编译此函数，协调器不编译
  case SAMPLEAPP_BLIND_CLUSTERID://说明是盲节点发送来的数据，只有参考节点接收盲节点的信息
...
//  HalUARTWrite(0, pkt->cmd.Data, pkt->cmd.DataLength);   
    ...
    {   
      ...
      //      rssi_s[0] ='A';
      rssi_s[0] = val/100+0x30;//ASCII编码 0x30对应的是数字 0。这里加上这个数就是要显示数字字符
...
//后期参考节点在发送数据的时候，必须发送的是参考节点与盲节点之间的距离值
{
  ...
  */
  ...
  if ( AF_DataRequest( &SampleApp_p2p_DstAddr, //发送目的地址+端点地址+传送模式
                      ...
                      SAMPLEAPP_ROUT_CNODE_CLUSTERID,//已经定义（系统或自定义）的簇的ID，用来表示发送的数据标识，让接收函数可以用来分类
                      3,//发送数据的长度
                      rssi_s,//发送数据缓冲区，即数据值
